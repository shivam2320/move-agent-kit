...
export const parser = StructuredOutputParser.fromZodSchema(
  z.object({
    isSupraReadQuery: z
      .boolean()
      .describe("Query requires reading data from Supra blockchain"),
    isWriterQuery: z
      .boolean()
...
export const prompt = PromptTemplate.fromTemplate(
  `
    ...
    2. Determine if the query requires Supra read operations, tweet/post write operations, or posting on X/Twitter
...
export const managerNode = async (state: typeof StateAnnotation.State) => {
  ...
  const { isSupraReadQuery, isWriterQuery, isXPostQuery } = result;
