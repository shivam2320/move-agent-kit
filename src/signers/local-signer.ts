...
export class LocalSigner extends BaseSigner {
  ...
  async signTransaction(
    ...
    const senderAuthenticator = SupraClient.createSignedTransaction(
      this.account,
      transaction
    );
    ...
    return {
      senderAuthenticator:
        senderAuthenticator as unknown as TxnBuilderTypes.AccountAuthenticatorEd25519,
    };
  }

  async sendTransaction(transaction: Uint8Array): Promise<HexString> {
    const submittedTx = await this.supra.sendTxUsingSerializedRawTransaction(
      this.account,
      transaction,
      {
